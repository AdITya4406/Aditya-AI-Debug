...
LEARNING_RATE        = 0.005
EPOCHS               = 600
CONFIDENCE_THRESHOLD = 0.40                                                #Bug 5 fix... 
...
def preprocess(text: str) -> list:
    ...
    return [lemmatizer.lemmatize(tok) for tok in tokens if tok.isalpha()]   # Bug 2 fix...
...
class VanillaRNN:
    ...
    def backward(self, probs: np.ndarray, target_idx: int) -> float:
        ...
        for t in reversed(range(n)):
            dtanh  = (1.0 - self._hs[t + 1] ** 2) * d_h   # tanh derivative   #Bug 3+4 fix...
            d_bh  += dtanh
...
def _softmax(x: np.ndarray) -> np.ndarray:
    e_x = np.exp(x - np.max(x))                                                #Bug 1 fix...
    return e_x / e_x.sum()
